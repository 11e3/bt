...
def main():
    ...
    for n in range(2, 6):  # 2, 3, 4, 5 coins
        print(f"\n{'=' * 100}")
        print(
            f"{n}-COIN COMBINATIONS (C(5,{n}) = {len(list(combinations(all_symbols, n)))} combinations)"
        )
        print(f"{'=' * 100}\n")
        ...
        for combo in combinations(all_symbols, n):
            ...
            try:
                ...
                print(
                    f"CAGR: {result['cagr']:>7.2f}%, MDD: {result['mdd']:>7.2f}%, Sharpe: {result['sharpe']:>5.2f}"
                )
        ...
        if combo_results:
            ...
            for r in combo_results[:5]:
                print(
                    f"{r['symbols_str']:<25} {r['cagr']:>10.2f}%  {r['mdd']:>10.2f}%  {r['sharpe']:>10.2f}"
                )
    ...
    for r in all_results[:10]:
        print(
            f"{r['symbols_str']:<25} {r['n_coins']:<5} {r['cagr']:>10.2f}%  {r['mdd']:>10.2f}%  {r['sharpe']:>10.2f}"
        )
